package aula08;

public class Aula08 {

    public static void main(String[] args) {

        Lutador l[] = new Lutador[6];
        l[0] = new Lutador("Pretty Boy", "Franca", 31, 1.75f, 68.9f, 11, 3, 1);
        l[1] = new Lutador("Putscript", "Brasil", 29, 1.68f, 57.8f, 14, 2, 3);
        l[2] = new Lutador("Snapshadow", "EUA", 35, 1.65f, 80.9f, 12, 2, 1);
        l[3] = new Lutador("Dead Code", "Australia", 28, 1.93f, 81.6f, 13, 0, 2);
        l[4] = new Lutador("UFOCobol", "Brasil", 37, 1.70f, 119.3f, 5, 4, 3);
        l[5] = new Lutador("Nerdaard", "EUA", 30, 1.81f, 105.7f, 12, 2, 4);

        //testando luta peso leve
        Luta ufc1 = new Luta();
        ufc1.marcarLuta(l[0], l[1]);
        ufc1.lutar();
        l[0].status();
        l[1].status();
        //testando luta peso medio     
        Luta ufc2 = new Luta();
        ufc1.marcarLuta(l[2], l[3]);
        ufc1.lutar();
        l[2].status();
        l[3].status();
        //testando luta peso pesado       
        Luta ufc3 = new Luta();
        ufc1.marcarLuta(l[4], l[5]);
        ufc1.lutar();
        l[4].status();
        l[5].status();
        //testando lutadores iguais
        Luta ufc4 = new Luta();
        ufc1.marcarLuta(l[1], l[1]);
        ufc1.lutar();
        //testando lutadores de categorias diferentes
        Luta ufc5 = new Luta();
        ufc1.marcarLuta(l[3], l[4]);
        ufc1.lutar();

    }
}
